...
#define OBJ_ENCODING_LISTPACK /*紧凑列表*/
/*-- Redis对象 --*/
...
有数据可读时,调用epoll_wait就可以得到通知。但是事件的通知的模式有两种：

- LevelTriggered：简称LT。当FD有数据可读时，会重复通知多次，知道数据处理完成，是Epoll的默认模式
- EdgeTriggered：简称ET。当FD有数据可读时，只会被通知一次，不管数据是否处理完成。

使用ET模式时，就绪fd从就绪列表移除后不会添加回去，要解决有两种方案：

- 手动调用epoll_ctl函数将fd状态手动修改
- 被通知后使用非阻塞IO循环读取fd

### 基于Epoll模式的web服务流程

![image-20241003110401258](redis.assets/image-20241003110401258.png)

### 信号驱动IO

第一阶段注册信号并设置回调，FD就绪时，会发出信号通知用户，期间用户应用可以执行其他业务。

![image-20241003110744079](redis.assets/image-20241003110744079.png)

有大量IO操作时，信号多，待处理的队列可能溢出

信号交互要频繁切换内核和用户空间，耗费性能

### 异步IO

![image-20241003111350028](redis.assets/image-20241003111350028.png)

两个阶段都是非阻塞的。但是需要手动控制并发，防止内核任务过重。

![image-20241003111546042](redis.assets/image-20241003111546042.png)
...
- 核心业务处理为单线程
- 整个Redis为多线程
- Redis4.0引入多线程异步处理耗时长的任务
- Redis6.0核心网络模型引入多线程,进一步提高对于多核CPU的利用率

为什么选择单线程?

- Redis主要操作为内存操作,执行速度非常快,性能瓶颈是网络延迟而不是执行速度.多线程不会带来巨大提升
- 引入多线程会导致上下文切换,带来不必要的开销
- 引入多线程会导致线程安全问题,需要引入安全锁等手段,实现复杂度增高,性能变差。

Redis针对不同平台设计了统一的接口的不同实现

![image-20241003183721231](redis.assets/image-20241003183721231.png)

部分代码实现

![image-20241004214351670](redis.assets/image-20241004214351670.png)

读取并处理请求

![image-20241004220137389](redis.assets/image-20241004220137389.png)

Redis整体处理流程，其中命令解析和命令回复处理两个消耗IO的部分被设计成了多线程

![image-20241004220651785](redis.assets/image-20241004220651785.png)
