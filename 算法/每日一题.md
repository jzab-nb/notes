...
思路: 先对区间起点进行排序，然后定义双指针，一个标记大区间的起点，一个标记大区间的终点。然后遍历区间数组

当一个区间的起点小于等于大区间的终点时，进行合并，新终点是两个终点中较大的一个。

当一个区间的起点大于大区间的终点时，则旧的大区间就是结果的一部分，更新起点与终点。
...
使用栈，左括号入栈，遍历到右括号判断是否与栈顶左括号匹配，不匹配返回False，匹配则栈顶出栈
...
思路: 使用两个数组记录行和列，出现元素则对应位置1，遍历完毕后查看两个记录数组是否有0

```java
public boolean checkValid(int[][] matrix) {
    int n = matrix.length;
    int[][] row = new int[n][n];
    int[][] colnum = new int[n][n];
    ...
    for(int i=0;i<n;i++){
        for(int j=0;j<n;j++){
            if(row[i][j] != 1 || colnum[i][j] != 1) return false;
        }
    }
    return true;
}
