...
1. 使用```Map<String,Map<String,Integer>>```来记录机票的数据，key为出发站点，value为终点站点和数量的集合，数量为0时代表所有到这个终点站的都走过，这么做主要是为了防止循环
2. 递归函数拥有布尔返回值，第一次遇到结果时返回即可
3. 为了保证字典序，终点集合使用TreeMap来存放，会按照key排序
...
class Solution {
    ...
    public List findItinerary(List<List<String>> tickets) {
        for(List<String> ticket: tickets){
            ...
            // TreeMap保证升序
            Map<String, Integer> body = map.getOrDefault(start, new TreeMap<>());
            map.put(start, body);
            body.put(end,body.getOrDefault(end, 0)+1);
        }
        result.add("JFK");
        select("JFK", tickets.size());
        return (List) result;
    }
    // 起始站点和需要的节点数
    public boolean select(String start, int need){
        if(need==0) return true;
        // 循环出发站点对应的终点站
        Map<String, Integer> targets = map.get(start);
        // 当前站点出发没有终点站,返回false
        ...
        for (Map.Entry<String, Integer> entry : targets.entrySet( )) {
            ...
            if(entry.getValue()!=0){
                ...
                // 如果遍历到一种结果直接返回
                if(select(entry.getKey(), need-1)) return true;
                // 回溯
                entry.setValue(entry.getValue( )+1);
                result.removeLast();
            }
...
class Solution {
    ...
    public boolean select(String start, int need){
        ...
        for(int i=0;i<used.length;i++){
            if(!used[i] && tickets.get(i).get(0).equals(start)){
                result.add(tickets.get(i).get(1));used[i] = true;
                if(select(tickets.get(i).get(1), need-1)) return true;
                result.removeLast(); used[i] = false;
            }
        }
        return false;
    }
...
在一个n*n的盘子上尽可能的多摆皇后，返回所有解法

规则：同一行不能有两个，同一列不能有两个，斜向上不能有两个

思路：用二维数组记录状态，按照规则判断能否放下并回溯
...
class Solution {
    ...
    public void select(int i, int n){
        if(i==n){
    ...
    public boolean isValid(int i,int j,int n){
        ...
        // 右上方不能有
        for(int ti=i-1,tj=j+1;ti>=0 && tj<n;ti--,tj++){
            if(board[ti][tj]=='Q') return false;
        }
        return true;
    }
    ...
    public boolean select(){
        ...
        for(int j=0;j<9;j++){
            ...
            for(int i=0;i<9;i++){
                ...
                // 九个数字都用过了还没找到答案,返回false
                return false;
            }
        }
        // 三层都遍历完返回真(只有continue的时候会来到这里,也就是所有位置都填满了)
        return true;
    }
    // 在i,j位置上放n是否合法
    public boolean isValid(int i,int j, int n){
        for(int k=0;k<9;k++){
            if(board[k][j]-'0'==n || board[i][k]-'0'==n) return false;
        }
        int bi = (i/3)*3;
        int bj = (j/3)*3;
        for(int x=bi;x<bi+3;x++){
            for(int y=bj;y<bj+3;y++){
                if(board[x][y]-'0'==n) return false;
            }
        }
        return true;
    }
...
class Solution {
    public int wiggleMaxLength(int[] nums) {
        // 思路: 删除单调坡度上的值,保留峰值
        int preDiff=0; // 上一次的差值,在0号元素前虚拟一个和他相同的元素,初始差值是0
        int nextDiff=0; // 下一次的差值,每次计算
        // 默认存在一个峰值
        int result=1;
        // 遍历每个值
...
class Solution {
    public int wiggleMaxLength(int[] nums) {
        // 动态规划, dp[i][0]表示第i个数做山峰,dp[i][1]表示第i个数做山谷
        // dp[i][0] = max(dp[i][0], dp[j][1]+1) 其中0 < j < i且nums[j] < nums[i],表示j是上一个山谷
...
dp数组含义: dp[i]的含义是以nums[i]结尾的连续数组的最大和是多少
...
class Solution {
    public int maxSubArray(int[] nums) {
        ...
        // dp数组可以用一个变量代替,dp[i]的含义是以nums[i]结尾的连续数组的最大和是多少,然后返回dp中的最大值
        int dp = nums[0];
        // 记录结果的最大值
        int result = dp;
        // 遍历数组
        for(int i=1;i<nums.length;i++){
            // 要么是当前位加上前面的和,要么只是当前位
...
class Solution {
    public int maxProfit(int[] prices) {
        // 贪心: 将总的利润拆分,前一天买入第二天就卖出
        int result = 0;
        // 从第二天开始遍历
        for(int i=1;i<prices.length;i++){
            // 当天的利润为当前的价格减去前一天的价格
            int now = prices[i]-prices[i-1];
            // 当天利润大于0时才记录结果
            result+=now>0?now:0;
        }
        return result;
...
dp数组含义: hold当前持有股票的最大利润,cash第i天不持有股票的最大利润

状态转移方程：cash = Math.max(hold+i, cash); hold = Math.max(cash-i, hold );

```java
class Solution {
    public int maxProfit(int[] prices) {
        // 动态规划:dp[i][0] 第i天持有股票的最大利润,dp[i][1] 第i天不持有股票的最大利润
        // 可以简化成两个变量
        int cash,hold;
        ...
        for(int i: prices){
            int preCash = cash;
            int preHold = hold;
            // 当天不持有股票: 前一天有,卖了,前一天没有,不买
            cash = Math.max(preHold+i, preCash);
            // 当天持有股票: 前一天没有,买了,前一天有,不卖
            hold = Math.max(preCash-i, preHold);
        }
...
class Solution {
    public boolean canJump(int[] nums) {
        ...
        for(int i=1;i<nums.length && i<=max;i++){
            // max 为旧的max和i+nums[i]的最大值
            max = Math.max(max,i+nums[i]);
        }
        // 能跳的最大范围比长度的最后一位大,代表能走到最后
...
思路：同时记录最大距离和当前🙅‍的最大距离

```java
class Solution {
    public int jump(int[] nums) {
        // 记录当前步能达到的最远距离和下一步能达到的最远距离,当前步走到头还不能到达数组末尾时,再步数+1
        int curMax = 0,nextMax=0,step=0;
        // 右边界向左调一位,这样可以免去特殊情况的判断,
        // 因为当i走到倒数第二位时,如果恰好是这一步的最后一个,那结果就是步数+1
        // 如果不是这一步的最后一个,那么这一步肯定覆盖了最后一位,所以无需特判
        for(int i=0;i<nums.length-1;i++){
            // 随时更新最大覆盖距离
            nextMax = Math.max(nextMax, i+nums[i]);
            // 走到当前步的尽头,步数+1,更新当前步的最远距离
            if(i==curMax){
                curMax = nextMax;
                step++;
            }
...
class Solution {
    public int largestSumAfterKNegations(int[] nums, int k) {
        // 按照绝对值排序
        // java中的排序函数默认升序,两数比较返回1代表左边的数大,左边的数靠后,右边的数靠前,否则相反
        nums = IntStream.of(nums)
                .boxed()
                // a的绝对值大于b的绝对值时返回负数,所以,a的绝对值更小时a向右排,绝对值最小的靠右
...
贪心思路: 对每天的剩余油量进行累加,加到第i天出现负数了则说明从当前位置开始无法走完,则更新起点到i+1

同时记录总的剩余和,若总的为负数则肯定无法到达

```java
public int canCompleteCircuit(int[] gas, int[] cost) {
    // 从头开始累加
    int start=0;int sum=0;int totalSum=0;
...
思路:先只考虑自己左边的,再只考虑自己右边的,最后两者取最大值
...
public int candy(int[] ratings) {
    ...
    // 先从左向右遍历
    for(int i=1;i<n;i++){
        temp[i] = ratings[i]>ratings[i-1]?temp[i-1]+1:1;
    }

    result = Math.max(rightRecord,temp[n-1]);
    // 再从右向左遍历同时得出结果
    for(int i=n-2;i>=0;i--){
        rightRecord = ratings[i]>ratings[i+1]?rightRecord+1:1;
        result+=Math.max(rightRecord,temp[i]);
    }
    return result;
}
```
...
贪心思路: 20元时先用10元的，没有10元的再用5元的，因为5元的更通用。

```java
class Solution {
    public boolean lemonadeChange(int[] bills) {
        return select(bills,0,0,0,0);
    }

    public boolean select(int[] bills, int five, int ten, int twenty, int start){
        for(int i=start;i<bills.length;i++){
            int num = bills[i];
            // 5和10的情况直接减,不需要考虑其他情况
            if(num==5){
                five++;
            }else if(num==10){
...
思路：出现两个维度时，先只考虑一个维度，按照身高排序后，优先找身高最高的插入队列，不会影响之前的结果

```java
public int[][] reconstructQueue(int[][] people) {
    // 知道每个人的身高和前面比他高的人的数量
    // 先按身高降序排序,再按照前面比它高的人的个数升序排序
    Arrays.sort(people, Comparator.comparingInt((int[] a) -> -a[0]).thenComparingInt(a -> a[1]));
    LinkedList<int[]> list = new LinkedList<>( );
    // 遍历到某个数时,前面比他高的肯定已经插入完了,直接按照位置插入即可
...
![image-20241127181443706](%E4%BB%A3%E7%A0%81%E9%9A%8F%E6%83%B3%E5%BD%95.assets/image-20241127181443706.png)

用上一题的思路去思考,射的箭的数目就是不重叠区间的数目,总数去掉这个数字,得到的就是要移除的区间
...
从一个长字符串中尽可能多的划分区间,让相同的字母在同一个区间内

思路: 先统计所有字母最后出现的位置

再在遍历的时候,更新当前区间内的字母最后出现的位置也就是右端点,
...
class Solution {
    public List<Integer> partitionLabels(String s) {
        ...
        int[] mem = new int[26];
        // 初始化最远距离
        for(int i=0;i<s.length();i++) mem[s.charAt(i)-'a'] = i;
        int max_d=-1;
        int last=-1;
        // 第二次遍历,寻找答案
        for(int i=0;i<s.length();i++){
            ...
            int c = s.charAt(i)-'a';
            // 更新最远距离
            max_d = Math.max(mem[c],max_d);
            // 前面出现过的所有字母的最远距离就是这里,那么返回当前的值
            if(max_d==i){
                res.add(i-last);
                last = i;
            }
...
public int[][] merge(int[][] intervals) {
    ...
    for (int[] interval : intervals) {
        // 新区间的起点比当前大区间的终点靠左,可以合并
        if(interval[0]<=end){
            end = Math.max(interval[1],end);
        }
        else{
            // 否则就区间加入到结果中,开始一个新的区间
...
找一个比n小的从左到右全部单调递增的数字

从右向左遍历,找到逆序对,将左边的数-1,然后后面的全部变成9
...
思路: 使用后续遍历,先遍历左右孩子节点,确定了左右孩子的状态后,根据孩子的状态确定当前节点的状态
...
class Solution {
    ...
    // 每个节点的状态: 1.有摄像头 2.有覆盖 3.无覆盖
    public int dfs(TreeNode root,boolean isRoot){
        if(root==null) return 2;
        int left = dfs(root.left,false);
        int right = dfs(root.right,false);
        // 子节点有未被覆盖的,本节点必须放,子节点被覆盖,但是当前节点是根,也要放
        if(left==3 || right==3 || (left==2 && right==2 && isRoot)){
            cnt++;
            return 1;
        }
        // 子节点有摄像机,本节点被覆盖
        if(left==1 || right==1) return 2;
        // 左右都有覆盖,并且当前不是根几点,本节点可以不放
        if(left==2 && right==2 && !isRoot) return 3;
        return 1;
    }
}
```
...
public int fib(int n) {
    // 1.确定dp数组的含义: dp[i]: 第i项数列值
    // 2.确定递推公式: dp[i] = dp[i-2]+dp[i-1];
    // 3.确定递推初始值: dp[0]=0 dp[1]=1
    // 4.确定遍历方向: 从左向右
    int left=0,right=1;
    while(n>0){
        right = left+right;
...
一个m*n的二维网格,只能向右或向左走,有多少种不同的路径

```java
public int uniquePaths(int m, int n) {
    // 1. dp[i][j] 代表走到当前位置的路径数
...
同上,但有障碍物的地方不能走,所以路径数是0

```java
public int uniquePathsWithObstacles(int[][] obstacleGrid) {
    // 1. dp[i][j] 代表走到当前位置的路径数
    // 2. dp[i][j]  = dp[i-1][j] + dp[i][j-1] || 0
    // 3. 初始化dp[0][0] = 1
    // 4. 从左到右,从上到下遍历
    // 5. 举例:
    //    1 1
...
因为要求的数是2-58,范围很小,可以直接初始化完dp数组,计算的时候直接取值

动态规划五部曲:

1. dp[i] 的含义: 拆分i可以得到的最大乘积
2. 递推公式: dp[i] = max( (i-j)\*j, dp[i-j]\*j ) 一种情况是只拆成两个数，另一种是拆成三个及以上的数
3. dp初始化: dp[2] = 1 2只能拆成1+1
4. 遍历顺序: 从左向右,因为i的拆分需要用到比i小的j
5. 举例推导

```java
class Solution {
    public static int[] result = new int[59];
...
![image-20241204183616148](%E4%BB%A3%E7%A0%81%E9%9A%8F%E6%83%B3%E5%BD%95.assets/image-20241204183616148.png)
...
1. 定义dp数组含义：二维数组 dp\[i\]\[j\]
   - i：可以选择第i个物品了
   - j：共有j个容量
   - dp\[i\]\[j\]: 最大容量为j且能选前i个物品时，可以获得的最大价值
2. 定义递推公式：
   - 当前行不选：dp\[i-1\]\[j\]，当前行不选，上一行不需要给当前行留空间
   - 当前行选择：dp\[i-1\]\[j-weight[i]\]+value[i]，上一行需要留出足够当前行选择的空间来，用上一行的价值加上当前选择的物品的价值
3. 初始化：当j=0时，初始化所有dp\[i\]\[0\] 为0，当i=0时，初始化第0个物品的价值当i>=物品的重量的时候
4. 遍历方向：根据公式，需要从上到下，从左到右
